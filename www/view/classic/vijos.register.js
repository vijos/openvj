(function(window, vj, undefined)
{
	function calcStrength(password)
	{
		...
		if (result != null) { strength++; if (result.length > 3) strength++; }
		
		//若含有大写字母或更多大写字母则增加权值
		result = password.match(/[A-Z]/g);
		if (result != null) { strength++; if (result.length > 2) strength++; if (result.length > 3) strength++; }
		
		//若含有特殊符号或更多特殊符号则增加权值
		result = password.match(/[^a-zA-Z0-9]/g);
		if (result != null) { strength++; if (result.length > 1) strength++; if (result.length > 2) strength++; if (result.length > 3) strength++; }
